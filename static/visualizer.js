...
const viz = {
  ...
  // Defined in drill.js — reuse if available, else define locally
  function fetchWithTimeout(url, options = {}, ms = 5000) {
    if (typeof window._fetchWithTimeout === 'function')
      return window._fetchWithTimeout(url, options, ms);
  ...
  async function vizLoadFile(filename) {
    const url = filename ? `/api/ranges?file=${encodeURIComponent(filename)}` : '/api/ranges';
